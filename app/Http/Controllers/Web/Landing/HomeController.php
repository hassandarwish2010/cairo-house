...
//        $new->full_image = asset('public/uploads/' . $new->image);
      ...
      $new->en_title = clean_limit($new->en_title, 15);
      $new->ar_title = clean_limit($new->ar_title, 35);
      $new->en_details = clean_limit($new->en_details, 230);
      $new->ar_details = clean_limit($new->ar_details, 250);
      $new->month = Carbon::parse($new->created_at)->format('M');
        $new->year = Carbon::parse($new->created_at)->format('Y');
      $new->day = Carbon::parse($new->created_at)->day;
    }

    return view('web.landing.index', compact('banners', 'products', 'news'));
  }
}
